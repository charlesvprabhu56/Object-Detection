...
def crossover(parent1, parent2, fitness, crossover_rate=0.7):
    currentfit = fitness
    worstfit = np.max(fitness)
    meanfit = np.mean(fitness)
    r = currentfit / ((worstfit + (meanfit + currentfit)) / 100)
    if r < crossover_rate:
...
def mutate(doll, fitness, mutation_rate=0.1, mutation_strength=0.5):
    currentfit = fitness
    worstfit = np.max(fitness)
    meanfit = np.mean(fitness)
    r  = currentfit / (worstfit + (meanfit + currentfit) / 100)
...
# Modified Dollmaker Optimization Algorithm (MDOA) Starting Line No. 16
def Proposed(population, objective_function, VRmin, VRmax, generations):
    ...
    for gen in range(generations):
        ...
        for i in range(0, len(parents), 2):
            ...
            child1, child2 = crossover(np.array(parent1), np.array(parent2), fitness)
            next_population.append(mutate(child1), fitness)
